// Accessible pagination for the responsive Electro benefits carousel.
(() => {
  ...
  pagination.replaceChildren(...cards.map((card, index) => {
    ...
    button.addEventListener('click', () => track.scrollTo({left: index * step(), behavior: motion()}));
    return button;
  }));
  function update() {
  ...
  previous.addEventListener('click', () => track.scrollBy({left: -step(), behavior: motion()}));
